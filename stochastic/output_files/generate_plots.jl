...
for k in 2:k_max
    ...
    dc_expected_load_shed[k-1] = solution[1]["expected_load_shed"]
    dc_ac_load_shed[k-1] = (typeof(solution[1]["ac_load_shed"]) == Void) ? dc_load_shed[k-1] : solution[1]["ac_load_shed"]
    dc_expected_ac_load_shed[k-1] = (typeof(solution[1]["expected_ac_load_shed"]) == Void) ? dc_load_shed[k-1] : solution[1]["expected_ac_load_shed"]
    dc_times[k-1] = solution[1]["time"]
    dc_iterations[k-1] = solution[1]["iterations"]

    soc1_load_shed[k-1] = solution[2]["load_shed"]
    soc1_expected_load_shed[k-1] = solution[2]["expected_load_shed"]
...
plt = Axis([
            ...
            Plots.Linear(k_values, soc1_load_shed*100, markSize=0.5, legendentry=L"SOC"),
            Plots.Linear(k_values, dc_ac_load_shed*100, markSize=0.5, legendentry=L"DC(AC)"),
            Plots.Linear(k_values, soc1_ac_load_shed*100, markSize=0.5, legendentry=L"SOC(AC)"),
...
plt = Axis([
            Plots.Linear(k_values, dc_expected_load_shed*100, markSize=0.5, legendentry=L"DC"),
            Plots.Linear(k_values, soc1_expected_load_shed*100, markSize=0.5, legendentry=L"SOC"),
            Plots.Linear(k_values, dc_expected_ac_load_shed*100, markSize=0.5, legendentry=L"DC(AC)"),
