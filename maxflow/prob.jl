...
function post_ots{T}(pm::GenericPowerModel{T})
    ...
    PowerModels.variable_complex_voltage_on_off(pm) # adds phase angle variables for DC
    variable_active_generation(pm)
    variable_reactive_generation(pm) # does nothing for DC
    variable_load_shed(pm) 
    PowerModels.variable_active_line_flow(pm) 
    PowerModels.variable_reactive_line_flow(pm) # does nothing for DC

    objective_min_load_shed(pm)
    ...
    for (i, branch) in pm.set.branches
        PowerModels.constraint_active_ohms_yt_on_off(pm, branch)
        PowerModels.constraint_reactive_ohms_yt_on_off(pm, branch)

        PowerModels.constraint_phase_angle_difference_on_off(pm, branch)
        PowerModels.constraint_thermal_limit_from_on_off(pm, branch)
        PowerModels.constraint_thermal_limit_to_on_off(pm, branch)
    end
...
function variable_line_indicator{T}(pm::GenericPowerModel{T})
    @variable(pm.model, 0 <= line_z[l in pm.set.branch_indexes] <= 1, start = PowerModels.getstart(pm.set.branches, 1, "line_z_start", 1.0))
    return line_z 
end 
