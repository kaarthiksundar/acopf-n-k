...
if ARGS[3] == "DC"
    args["model"] = DCPPowerModel
else 
    args["model"] = SOCWRPowerModel
end
args["cut"] = ARGS[4]
...
master_problem(file = args["file"], k = args["k"], solver = GurobiSolver(LogToConsole=0), model_constructor = args["model"], cut_constructor = args["cut"])
